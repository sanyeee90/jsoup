...
public class DocumentTest {
	...
	public void testTitles() {
		...
		Document withTitle = Jsoup
				.parse("<title>First</title><title>Ignore</title><p>Hello</p>");
	...
	public void testOutputEncoding() {
		...
		assertEquals(Entities.EscapeMode.base, doc.outputSettings()
				.escapeMode());
		assertEquals("<p title=\"&#x3c0;\">&#x3c0; &amp; &lt; &gt; </p>", doc
				.body().html());

		doc.outputSettings().escapeMode(Entities.EscapeMode.extended);
		assertEquals("<p title=\"&pi;\">&pi; &amp; &lt; &gt; </p>", doc.body()
				.html());
	}
	...
	public void testNormalisesStructure() {
		Document doc = Jsoup
				.parse("<html><head><script>one</script><noscript><p>two</p></noscript></head><body><p>three</p></body><p>four</p></html>");
		assertEquals(
	...
	public void testClonesDeclarations() {
		Document doc = Jsoup
				.parse("<!DOCTYPE html><html><head><title>Doctype test");
		Document clone = doc.clone();
	...
	public void testLocation() throws IOException {
		...
		assertEquals(
				"http://www.yahoo.co.jp/_ylh=X3oDMTB0NWxnaGxsBF9TAzIwNzcyOTYyNjUEdGlkAzEyBHRtcGwDZ2Ex/",
				baseUri);
		in = new ParseTest().getFile("/htmltests/nyt-article-1.html");
		doc = Jsoup
				.parse(in, null,
						"http://www.nytimes.com/2010/07/26/business/global/26bp.html?hp");
	...
	public void testMetaCharsetUpdateNoCharset() {
		...
		assertEquals(charsetUtf8, docNoCharset.select("meta[charset]").first()
				.attr("charset"));
	...
	public void testMetaCharsetUpdateXmlUtf8() {
		...
		final String xmlCharsetUTF8 = "<?xml version=\"1.0\" encoding=\""
				+ charsetUtf8 + "\">\n" + "<root>\n" + " node\n" + "</root>";
	...
	public void testMetaCharsetUpdateXmlIso8859() {
		...
		final String xmlCharsetISO = "<?xml version=\"1.0\" encoding=\""
				+ charsetIso8859 + "\">\n" + "<root>\n" + " node\n" + "</root>";
	...
	private Document createHtmlDocument(String charset) {
		...
		doc.head().appendElement("meta").attr("name", "charset")
				.attr("content", charset);

		return doc;
	}

	private Document createXmlDocument(String version, String charset,
			boolean addDecl) {
	...
	@Test
	public void testBasicNormaliseMechanism() {
		Document doc = new Document("");
		Document normalisedDoc = doc.normalise();

		assertTrue(normalisedDoc.outerHtml().contains("<html>")
				&& normalisedDoc.outerHtml().contains("<head>")
				&& normalisedDoc.outerHtml().contains("</head>")
				&& normalisedDoc.outerHtml().contains("<body>")
				&& normalisedDoc.outerHtml().contains("</body>")
				&& normalisedDoc.outerHtml().contains("</html>"));

	}

	@Test
	public void testBasicNormaliseMechanismWithMultipleElements() {
		String html = "<root><child></child><child></chidl></root>";
		Document doc = Jsoup.parse(html);
		doc.select("root").first().children().first()
				.before("<newChild></newChild>");
		Document normalisedDoc = doc.normalise();

		String normalisedHtml = normalisedDoc.outerHtml();
		...
		assertEquals(fullHtml, normalisedHtml);
